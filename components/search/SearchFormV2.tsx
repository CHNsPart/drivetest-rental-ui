...
import DriveTestCentreSelect from './DriveTestCentreSelect';
import RoadTestTypeSelect from './RoadTestTypeSelect';
import { Calendar, Car, Clock, MapPin, ChevronRight, Info, Check, Search } from 'lucide-react';
import { cn } from '@/lib/utils';
import { motion, AnimatePresence } from 'framer-motion';
...
export default function SearchFormV2({
  ...
  // Progress indicator for multi-step form (shown when expanded)
  const renderProgressIndicator = () => {
    return (
      <div className="mb-6 md:mb-8">
        <div className="flex items-center justify-between mb-2">
  ...
  // Horizontal compact search bar (initial state)
  const renderHorizontalSearchBar = () => {
    return (
      <div className="bg-white rounded-xl shadow-lg py-4 px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {/* First column - Test Type / Drive Test Centre */}
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Car className="h-5 w-5 text-brand-500" />
              <span className="text-sm font-medium text-gray-700">Test Type & Centre</span>
            </div>
            <div className="text-base text-gray-800 truncate">
              {formData.roadTestType || "Select test type"}
              {formData.driveTestCentreId && (
                <span className="block text-sm text-gray-600 truncate">
                  {CanadianLocations.find(c => c.id === formData.driveTestCentreId)?.name || ""}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="text-base text-gray-800 truncate">
              {formData.testDate ? new Date(formData.testDate).toLocaleDateString() : "Select date"}
              {formData.testTime && (
                <span className="block text-sm text-gray-600">{formData.testTime}</span>
              )}
            </div>
          </div>

          {/* Third column - Pickup & Dropoff */}
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <MapPin className="h-5 w-5 text-brand-500" />
            ...
            <div className="text-base text-gray-800 truncate">
              {formData.meetAtCentre 
                ? "Meet at centre" 
                : formData.pickupAddress || "Enter location"}
            </div>
          </div>
          ...
          <div className="flex items-center justify-center">
            <button
              ...
              className="w-full h-12 flex items-center justify-center bg-gradient-to-r from-brand-500 to-brand-400 hover:from-brand-500 hover:to-brand-600 text-white font-medium rounded-lg transition-all duration-300 shadow-md hover:shadow-lg"
            >
  ...
  return (
    <div className={`w-full rounded-2xl overflow-hidden shadow-xl border border-gray-200/50 bg-white backdrop-blur-sm transition-all ${className}`}>
      ...
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-8">
        {/* Toggle between compact and expanded form */}
        {!isExpanded ? (
          renderHorizontalSearchBar()
        ) : (
          <div className="transition-all duration-300">
            {/* Progress bar - only shown in expanded mode */}
            {renderProgressIndicator()}
      ...
      {/* Meet at center info box */}
      {formData.meetAtCentre && selectedCenter && currentStep === 3 && isExpanded && (
        <div 
          className="bg-gradient-to-r from-blue-50 to-brand-50 p-6 border-t border-blue-100 mb-5"
          style={{ opacity: 1, transform: 'translateY(0px)' }}
        >
          <div className="flex flex-col sm:flex-row">
            <div className="flex-shrink-0 mb-4 hidden sm:block sm:mb-0 sm:mr-4">
      ...
      {/* Collapse Form Button - Only show when expanded and not calculating */}
      {isExpanded && !isCalculating && !showPricing && (
        <div className="text-center mb-6">
          <button
            type="button"
            onClick={() => setIsExpanded(false)}
            className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500"
          >
